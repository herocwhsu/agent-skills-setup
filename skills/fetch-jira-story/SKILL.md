...
# When prompted:
#   Jira URL:  https://vivotek.atlassian.net
#   Username:  hero.hsu@vivotek.com   ← full email required for Jira Cloud
#   Password:  <API token>
#
# Resulting keychain entry:
#   service: agent-skills:jira-https---vivotek-atlassian-net
#   account: hero.hsu@vivotek.com

# Verify (safe — no value printed):
bash /path/to/agent-skills-setup/scripts/credentials/jira.sh verify
```

**Important:** Jira Cloud requires the full email address as the username. `hero.hsu` alone returns 401. Store and use `hero.hsu@vivotek.com`.
...
_JIRA_PASS=$(security find-generic-password -s "agent-skills:jira-https---vivotek-atlassian-net" -a "$_JIRA_USER" -w 2>/dev/null)
...
# https://confluence.vivotek.com → agent-skills:confluence-https---confluence-vivotek-com
_CONF_USER="hero.hsu"
_CONF_PASS=$(security find-generic-password -s "agent-skills:confluence-https---confluence-vivotek-com" -a "$_CONF_USER" -w 2>/dev/null)
...
if [ -z "$_CONF_PASS" ]; then
  ...
  curl -s -u "$_CONF_USER:$_CONF_PASS" \
    "https://confluence.vivotek.com/rest/api/content/$PAGE_ID?expand=body.storage,title" \
    | python3 -c "
import json, sys
