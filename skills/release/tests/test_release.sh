...
for gate in "OpenSpec" "Jira" "Apidog" "Tests"; do
  grep -q "$gate" $REPO_DIR/skills/release/readiness/IMPL.md \
...
grep -q "status: ready\|blocking_count: 0\|all checks pass" \
  $REPO_DIR/skills/release/readiness/IMPL.md \
...
for cat in "Incident" "Bug" "Regression" "Spec gap" "Enhancement" "Operational"; do
  grep -q "$cat" $REPO_DIR/skills/release/triage/IMPL.md \
...
grep -q "New.*proposal\|new.*OpenSpec\|new story\|new.*Jira" \
  $REPO_DIR/skills/release/triage/IMPL.md \
...
grep -q "release/bugfix" $REPO_DIR/skills/release/bugfix-spec/IMPL.md \
...
grep -q "regression\|testing-regression" $REPO_DIR/skills/release/bugfix-spec/IMPL.md \
...
grep -q "openspec_changes" $REPO_DIR/skills/release/archive-check/IMPL.md \
...
grep -q "status: complete\|status: incomplete" \
  $REPO_DIR/skills/release/archive-check/IMPL.md \
