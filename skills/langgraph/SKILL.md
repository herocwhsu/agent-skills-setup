...
LangSmith (cloud, free tier 5k traces/month):
```bash
...
Langfuse (self-hosted, preferred — no data leaves your infra):
```bash
...
@observe(name="audit-node")
def audit_node(state):
    langfuse_context.update_current_observation(
        metadata={"skill_name": "audit", "model": "claude-sonnet-4-6"}
    )
    ...
```

Note: LangSmith self-hosting requires an Enterprise license — use Langfuse for local self-hosted tracing.
Every node invocation appears as a span automatically when either backend is configured.
