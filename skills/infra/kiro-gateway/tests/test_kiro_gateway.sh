...
patch_exists_test() {
  ...
  local ixpatch="$(cd "$(dirname "$0")/.." && pwd)/patches/kiro-gateway-toolcall-index.patch"
  if [[ -f "$patch" ]] && grep -Fq "models_anthropic.py" "$patch" && grep -Eq 'role' "$patch" \
     && [[ -f "$nspatch" ]] && grep -Fq "_flatten_tool_namespaces" "$nspatch" \
     && [[ -f "$ixpatch" ]] && grep -Fq "assistant_tool_calls" "$ixpatch"; then
    echo "PASS: $name"; PASS=$((PASS+1))
  else
...
fix_guard_toolindex_missing_test() {
  ...
  out=$(HOME="$tmpdir" CANONICAL_DIR="$canon" \
    bash "$SCRIPT" __fix_guard 2>&1) || code=$?
  if [[ "$code" -ne 0 ]] && echo "$out" | grep -q 'tool-call index keying'; then
    echo "PASS: $name"; PASS=$((PASS+1))
  else
    echo "FAIL: $name (code=$code out=$out)"; FAIL=$((FAIL+1))
  fi
...
fix_guard_toolindex_missing_test "fix-guard aborts when tool-index fix is absent"
