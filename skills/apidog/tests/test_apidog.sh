...
grep -q 'STORY_DIR.*apidog/contract.md\|apidog/contract' \
  $REPO_DIR/skills/apidog/contract/IMPL.md \
...
for scenario in "Success" "Permission denied" "Unauthorized" "Rate limited" "Not found"; do
  grep -q "$scenario" $REPO_DIR/skills/apidog/mocks/IMPL.md \
...
grep -q "Permission cases\|permission" $REPO_DIR/skills/apidog/testcases/IMPL.md \
...
grep -q "gate\|before.*implementation\|implementation starts" \
  $REPO_DIR/skills/apidog/SKILL.md \
