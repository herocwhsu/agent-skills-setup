...
| Macro that the converter doesn't recognize | Unknown macros are silently dropped (no placeholder left in output). |
...
| **Cross-space page moves** | The Confluence REST API (`PUT /rest/api/content/{id}`) rejects moves across spaces with HTTP 500. Use fetch + upload instead — the source and destination spaces can differ. |
| **Upload aborts mid-pass-2 (XHTML error)** | `tree_upload.py` aborts on first content error. Stubs are already created. Build a `stub_map.json` from the live tree (title → new_id) and use a resume script that skips pages already at version ≥ 2 and falls back to a placeholder on XHTML failure. |
| **Formatting lost after upload** | The markdown conversion is lossy for rich Confluence pages (panels, layouts, tables-in-macros). For pages where fidelity matters, bypass markdown: fetch the original XHTML storage body via `GET /rest/api/content/{src_id}?expand=body.storage` and PUT it directly to the destination page, rewriting `ri:content-id` and `pageId=` references using the stub_map. |
| **Page title collision on upload** | Confluence Server/DC enforces unique titles per space. If stubs from a failed run remain, delete them before retrying. If the source tree has titles that clash with existing pages, rename them in `manifest.json` before upload. |
| **`ac:structured-macro` in PUT body → HTTP 501** | Confluence Server/DC rejects `ac:structured-macro` (code blocks, info panels) in storage-format PUT bodies with 501 on older Server versions. Use `<pre>...</pre>` for code blocks instead; strip `ac:structured-macro` from generated content before PUT. |
| **Unreconciled page → HTTP 501 on PUT** | Pages created via API but never viewed in a browser return `Unable to save changes to unreconciled page ContentId{id=...}`. Fix: open `/pages/viewpage.action?pageId=<id>` in a browser once (GET alone does not reconcile it), then retry the PUT. |
| **`&` inside `<pre>` → HTTP 400** | Storage format is XML — unescaped `&`, `<`, `>` inside `<pre>` blocks fail. Always escape to `&amp;`, `&lt;`, `&gt;` before PUT. |
| **Credential slug mismatch (Python vs bash)** | `cred_provider.py`'s `_slugify_url()` must NOT collapse consecutive dashes. BSD sed (macOS) does not support `\+` the way GNU sed does, so bash's `slugify_url` leaves triple dashes in place (`https://` → `https---`). A Python-side collapse produces a different key and credential lookup silently fails. |
