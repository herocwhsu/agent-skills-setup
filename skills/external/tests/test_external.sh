...
grep -q 'STORY_DIR.*external-deps.md\|external-deps.md' \
  $REPO_DIR/skills/external/deps/IMPL.md \
...
for item in "Can start" "Blocked"; do
  grep -q "$item" $REPO_DIR/skills/external/deps/IMPL.md \
...
grep -q "adapter\|Adapter" $REPO_DIR/skills/external/deps/IMPL.md \
  && grep -q "Internal interface" $REPO_DIR/skills/external/deps/IMPL.md \
...
grep -q "Provisional" $REPO_DIR/skills/external/deps/IMPL.md \
...
grep -q "blocking_count" $REPO_DIR/skills/external/deps/IMPL.md \
