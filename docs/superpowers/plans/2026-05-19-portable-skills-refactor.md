...
```bash
bash -c 'source lib/lib.sh && slugify_url https://example-org.atlassian.net'
```

Expected output: `https---example-atlassian-net`
...
```bash
bash -c 'source lib/lib.sh && service_slug jira https://example-org.atlassian.net'
```

Expected output: `jira-https---example-atlassian-net`

- [ ] **Step 5: Verify slug matches existing keychain entries**

```bash
security find-generic-password -s "agent-skills:jira-https---example-atlassian-net" -a "<user>@example.com" 2>&1 | grep -c svce
```
...
Remove all hardcoded example-org/<user> values. Use lib.sh helpers.
...
```bash
grep -E "example-org|hero\.hsu" skills/fetch-jira-story/SKILL.md
```
...
Removes example-org/<user> hardcoded references; reads JIRA_HOST etc.
...
```bash
grep -E "example-org|hero\.hsu" skills/fetch-page-to-markdown/SKILL.md
```
...
Removes example-org/<user> hardcoded references; reads CONFLUENCE_HOST
...
```bash
grep -E "example-org|hero\.hsu|VOR" skills/create-story-tasks/SKILL.md
```

Expected output: empty (no hardcoded refs). Note: example mentions like `EXAMPLE-100` in the file's prose are fine if they're labelled as examples — search above is for code references. If you find any literal `VOR` outside example narrative, replace with `<STORY-ID>` placeholder.
...
of hardcoding 'example-org' and 'VOR'.
"
```
...
Expected output (with your real values): `JIRA_HOST=example-org.atlassian.net, JIRA_USER=<user>@example.com, JIRA_PROJECT_KEY=VOR`.
...
- [ ] Three SKILL.md files contain zero hardcoded `example-org`/`<user>` strings — verify with `grep -E 'example-org|hero\.hsu' skills/*/SKILL.md`. Expected: empty.
...
- Three SKILL.md files reference user values via `${JIRA_HOST}` etc., no hardcoded `example-org` or `<user>`.
