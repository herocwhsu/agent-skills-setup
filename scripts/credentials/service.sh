...
CONFIG_FILE="${_SKILLS_RUNTIME_DIR}/config.sh"
...
service_def() {
  case "$1" in
    ...
    jira)
      ...
      ;;
    apidog)
      echo "apidog|||APIDOG_USER|APIDOG_PROJECT_ID:Apidog project ID"
      ;;
    anthropic)
...
write_config_kv() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    {
      echo "# ${CONFIG_FILE/#$HOME/~} — generated by setup-credentials.sh"
...
compute_slug() {
  ...
  source "${_SKILLS_RUNTIME_DIR}/lib.sh" 2>/dev/null
